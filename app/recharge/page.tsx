      ...
      <div className="max-w-4xl mx-auto px-4 pb-8">
        <Card className="p-6 mb-6 bg-gradient-to-r from-star-500/20 to-yellow-500/20 border-star-400/30">
          <div className="flex items-center justify-between">
            ...
            <Button variant="cosmos" onClick={() => setShowHistory(!showHistory)} className="flex items-center gap-2">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-cosmos-400">
                连接 Stripe 后将进入真实支付流程；未配置时会自动降级为演示模式。
              </p>
            </div>
          </div>
        </Card>
              ...
              <>
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button onClick={() => setPaymentMethod('wechat')} className={`p-3 rounded-lg border-2 transition-all ${paymentMethod === 'wechat' ? 'border-green-400 bg-green-400/10' : 'border-cosmos-700 hover:border-cosmos-600'}`}>
                      <Smartphone className="w-6 h-6 text-green-400 mx-auto mb-1" />
                      <p className="text-xs text-white">微信支付</p>
                    </button>
                    <button onClick={() => setPaymentMethod('alipay')} className={`p-3 rounded-lg border-2 transition-all ${paymentMethod === 'alipay' ? 'border-blue-400 bg-blue-400/10' : 'border-cosmos-700 hover:border-cosmos-600'}`}>
                      <QrCode className="w-6 h-6 text-blue-400 mx-auto mb-1" />
                      <p className="text-xs text-white">支付宝</p>
                    </button>
                    <button onClick={() => setPaymentMethod('card')} className={`p-3 rounded-lg border-2 transition-all ${paymentMethod === 'card' ? 'border-purple-400 bg-purple-400/10' : 'border-cosmos-700 hover:border-cosmos-600'}`}>
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="star" className="flex-1" onClick={handlePayment} disabled={isProcessing}>
                    {isProcessing ? (
                      <>
